...
export default function GalleryModal({ open, onClose, title, items }) {
  ...
  return (
    <AnimatePresence>
      {open && (
       <motion.div
          className="fixed inset-0 bg-black bg-opacity-95 flex flex-col items-center justify-start z-50 p-8"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          ...
          {/* Scrollable Content */}
          <div className="max-w-6xl w-full overflow-y-auto pr-2" style={{ maxHeight: 'calc(100vh - 80px)' }}>
